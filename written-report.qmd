...
linkedin_subset <- linkedin_subset |>
  filter(hourly_max_salary <= 150)
linkedin_subset <- linkedin_subset |>
  filter(hourly_max_salary > 10)
```
...
#| echo: false
set.seed(2)
linkedin_recipe2 <- recipe(per_applies ~ job_id + hourly_max_salary, data = linkedin_train) |>
  update_role(job_id, new_role = "ID") |> 
  step_dummy(all_nominal_predictors()) |>
  step_zv(all_predictors())
```
